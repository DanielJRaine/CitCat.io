...
const isGameOver = function() {
  ...
  let row1Sum = game.cells.slice(0,3).reduce(function(a,b) {
    return a + b;
  });
  let row2Sum = game.cells.slice(3,6).reduce(function(a,b) {
    return a + b;
  });
  let row3Sum = game.cells.slice(6).reduce(function(a,b) {
    return a + b;
  });
  
  let finalRowSum = [row1Sum, row2Sum, row3Sum];
  ...
  if(finalColumnSum._data.indexOf(3) >= 0 || finalRowSum.indexOf(3) >= 0 ||
